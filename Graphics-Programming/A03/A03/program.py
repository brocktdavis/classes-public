...
def make_new_pic(img):
    ...
    print "Actual Matrix: "+str(M)
    return img

def mark_features(img,blur=0):
    if blur!=0:
        img=cv2.GaussianBlur(img,(blur,blur),0,borderType=cv2.BORDER_REFLECT_101)
    orb=cv2.ORB()
...
img2=pic_1_a=make_new_pic(img1)
cv2.imwrite('output/pic_1_a.png',pic_1_a)

pic_1_b_1=mark_features(img1,blur=11)
pic_1_b_2=mark_features(img2,blur=11)
cv2.imwrite('output/pic_1_b_1.png',pic_1_b_1)
cv2.imwrite('output/pic_1_b_2.png',pic_1_b_2)

pic_1_c,M1=draw_mathces(img1,img2)
cv2.imwrite('output/pic_1_c.png',pic_1_c)
h1,w1=img1.shape[:2]
pic_1_e=cv2.warpPerspective(img1,M1,(w1,h1))
cv2.imwrite('output/pic_1_e.png',pic_1_e)

pic_1_f=np.float32(img2)-pic_1_e
pic_1_f-=np.min(pic_1_f)
pic_1_f/=np.max(pic_1_f)
pic_1_f*=255.999999
pic_1_f=np.uint8(pic_1_f)
cv2.imwrite('output/pic_1_f.png',pic_1_f)
